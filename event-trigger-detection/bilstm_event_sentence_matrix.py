...
def make_prediction(inps, in_sentence, sentence_ids, id2event):
    ...
    for i, in_sent in enumerate(in_sentence):
        ...
        in_sent = in_sent[start_sent:end_sent]
        j = 0
        while j < len(inp):
            event_id = inp[j]
            ...
            pred_labels_of_sent.append(event_type)
            j += 1

        result.append(events_of_sent)
        predicted_labels.append(pred_labels_of_sent)
...
def main(args):
    ...
    if not args.no_train:
        ...
        for epoch in range(args.num_epochs):
            ...
            for (batch, data) in enumerate(train_data_loader):
                ...
                preds, _ = make_prediction(logits, in_sentence, sentence_ids, id2event)
                labels, _ = make_prediction(labels, in_sentence, sentence_ids, id2event)
    ...
    preds, pred_labels = make_prediction(logits, in_sentence, sentence_ids, id2event)
    ...
    # Flatten the lists of lists to get all labels in one list
    true_labels_flat = [label for sublist in labels for label in sublist]
    pred_labels_flat = [label for sublist in pred_labels for label in sublist]

    # Get the list of all unique event types
    all_event_types = list(set(true_labels_flat) | set(pred_labels_flat))

    # Plot confusion matrix
    plot_confusion_matrix(true_labels_flat, pred_labels_flat, classes=all_event_types)
